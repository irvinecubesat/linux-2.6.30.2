...
static int camera_enable_gpio = -1;

static struct isi_platform_data isi_plat_data = {
   .image_hsize = 800,
   .image_vsize = 600,
   .prev_hsize = 320,
   .prev_vsize = 240,
   ...
   .cr1_flags = 0,
   .gpio_num_addr = &camera_enable_gpio,
   .gpio_active_val = 1,
};
...
static struct platform_device at91sam9260_isi_device = {
   ...
   .dev = {
      .platform_data = &isi_plat_data,
   },
};

void at91_set_isi_device_gpio(int gpio)
{
   camera_enable_gpio = gpio;
}
EXPORT_SYMBOL(at91_set_isi_device_gpio);

void at91_add_device_isi(unsigned long dma_phy_addr,
      unsigned long dma_phy_len)
{
   ...
   at91_set_B_periph(AT91_PIN_PB12, 1); 
   at91_set_B_periph(AT91_PIN_PB13, 1); 
   at91_set_B_periph(AT91_PIN_PB20, 1); 
   at91_set_B_periph(AT91_PIN_PB21, 1); 
   at91_set_B_periph(AT91_PIN_PB22, 1); 
   at91_set_B_periph(AT91_PIN_PB23, 1); 
   at91_set_B_periph(AT91_PIN_PB24, 1); 
   at91_set_B_periph(AT91_PIN_PB25, 1); 
   at91_set_B_periph(AT91_PIN_PB26, 1); 
   at91_set_B_periph(AT91_PIN_PB27, 1); 
   at91_set_B_periph(AT91_PIN_PB28, 1); 
   at91_set_B_periph(AT91_PIN_PB29, 1); 
   at91_set_B_periph(AT91_PIN_PB30, 1); 
   at91_set_B_periph(AT91_PIN_PB31, 1); 

   isi_plat_data.bootmem = (void*)dma_phy_addr;
   isi_plat_data.bootmemSize = dma_phy_len;

	platform_device_register(&at91sam9260_isi_device);
}

EXPORT_SYMBOL(at91_add_device_isi);
...
void __init at91_add_device_cf(struct at91_cf_data *data)
{
   ...
   // ***** DEBUG SFITZSIM - THIS IS NOT USED ON DEVBOARD
...
void __init at91_add_device_mmc(short mmc_id, struct at91_mmc_data *data)
{
	...
	if (data->slot_b) {
      ...
      // **** DEBUG SFITZSIM
      printk(KERN_ERR
					"******  MMC slot configuration attempt\n");
		/* CMD */
      // **** DEBUG SFITZSIM - normally uncommented
...
void __init at91_add_device_mci(short mmc_id, struct mci_platform_data *data)
{
	...
	// for (i = 0; i < 1; i++) {
		if (data->slot[i].bus_width) {
			/* input/irq */
			if (data->slot[i].detect_pin > 0) {
				at91_set_gpio_input(data->slot[i].detect_pin, 1);
				at91_set_deglitch(data->slot[i].detect_pin, 1);
			}
			if (data->slot[i].wp_pin > 0)
			...
			case 1:
	         // *** DEBUG SFITZSIM
            printk(KERN_ERR
					     "***** MCI slot configuration attempt\n");
...
#define TWI_PIN_SDA	AT91_PIN_PA23
#define TWI_PIN_SCL 	AT91_PIN_PA24
#define TWI_PIN_SDA1	AT91_PIN_PB20 // SDA1 and SCL1 may conflict with ISI.
#define TWI_PIN_SCL1	AT91_PIN_PB21

#define TWI_PIN_SDA2	AT91_PIN_PB17  // I2C-2 for Rev. 3
#define TWI_PIN_SCL2	AT91_PIN_PB18

// i2c-pl interface is removed from the <R3 boards.
#define TWI_PIN_SDA2_R2	AT91_PIN_PB5
#define TWI_PIN_SCL2_R2	AT91_PIN_PB4
// #define TWI_PIN_SDA2_R2	AT91_PIN_PA4
// #define TWI_PIN_SCL2_R2	AT91_PIN_PB6

#if defined(CONFIG_I2C_AT91) || defined(CONFIG_I2C_AT91_MODULE)

static struct platform_device_id twi_ip_type = {
   .driver_data = 2
};
...
static struct platform_device at91sam9260_twi_hw_device = {
	.name		= "at91_i2c",
	.id		= 0,
...
static struct i2c_gpio_platform_data pdata[] = {
   ...
   // Dev-board and PolySat boards : i2c-sb
   {
	.sda_pin		= TWI_PIN_SDA,	
	.sda_is_open_drain	= 1,
	.scl_pin		= TWI_PIN_SCL,
   ...
   // PolySat board ONLY : i2c-pl
   {
	.sda_pin		= TWI_PIN_SDA2,
	.sda_is_open_drain	= 1,
	.scl_pin		= TWI_PIN_SCL2,
	.scl_is_open_drain	= 1,
	.udelay			= 4,		/*  ~100 kHz */
   }, 
   // PolySat board ONLY Rev <= 2: i2c-pl
   {
	.sda_pin		= TWI_PIN_SDA2_R2,
	.sda_is_open_drain	= 1,
	.scl_pin		= TWI_PIN_SCL2_R2,
	.scl_is_open_drain	= 1,
	.udelay			= 4,		/*  ~100 kHz */
   }, 
};
...
static struct platform_device at91sam9260_twi_device[] = {
   {
	.name			= "i2c-gpio",
	.id			= 0,
	.dev.platform_data	= &pdata[0],
   },
   {
	.name			= "i2c-gpio",
	.id			= 1,
	.dev.platform_data	= &pdata[1],
   },
};

/* Change in i2c.  The second bus, old name i2c-1, was LS only.  If LS ever
 *  gets going again this should go into a LS kernel module.  Since it
 *  it conflicts with the ISI interface, it is removed from the default
 *  configuration.  It is still a hack, but the payload i2c bus is
 *  detected using the bus array size of 1.
 */
void __init at91_add_device_i2c(struct i2c_board_info *devices, int nr_devices)
{
   //hack, must fix later
   static int which = 1;
   struct i2c_gpio_platform_data *tmp_i2c;
   ...
   if (nr_devices != 1) {
	...
	/* pins used for TWI interface */
	   ...
	   platform_device_register(&at91sam9260_twi_hw_device);
      printk("*** HW i2c-0 driver\n");
#else
		at91_set_GPIO_periph(TWI_PIN_SDA, 1);		/* TWD (SDA) */
		at91_set_multi_drive(TWI_PIN_SDA, 1);
		at91_set_GPIO_periph(TWI_PIN_SCL, 1);		/* TWCK (SCL) */
		at91_set_multi_drive(TWI_PIN_SCL, 1);
		i2c_register_board_info(0, devices, nr_devices);
		platform_device_register(&at91sam9260_twi_device[0]);
      printk("*** GPIO i2c-0 driver\n");
#endif
	} else {
      if (BOARD_REV_NUM <= 2) {
         at91sam9260_twi_device[1].dev.platform_data = &pdata[2];
         at91_set_GPIO_periph( TWI_PIN_SDA2_R2, 1);
         at91_set_multi_drive( TWI_PIN_SDA2_R2, 1);	
         at91_set_GPIO_periph( TWI_PIN_SCL2_R2, 1);
         at91_set_multi_drive( TWI_PIN_SCL2_R2, 1);
      } else {
         at91_set_GPIO_periph( TWI_PIN_SDA2, 1);
         at91_set_multi_drive( TWI_PIN_SDA2, 1);	
         at91_set_GPIO_periph( TWI_PIN_SCL2, 1);
         at91_set_multi_drive( TWI_PIN_SCL2, 1);
      }

      i2c_register_board_info(1, devices, nr_devices);
      platform_device_register(&at91sam9260_twi_device[1]);
	}

   printk("*** called MOD at91_add_device_i2c\n");
}
...
void __init at91_add_device_i2c(struct i2c_board_info *devices, int nr_devices)
{
   ...
   printk("*** called ORIG at91_add_device_i2c");
}
#else
void __init at91_add_device_i2c(struct i2c_board_info *devices, int nr_devices) {}
#endif
...
void __init at91_add_device_spi(struct spi_board_info *devices, int nr_devices)
{
	...
	short enable_spi0 = 0;
	short enable_spi1 = 0;
   struct AtmelSPIMux *mux = NULL;
   struct AtmelSPIMuxData *muxData = NULL;
   struct AtmelSPIMuxDevReady *devRdy = NULL;

   // If board is Rev.3+, enable SPI mux enable line.
   if (BOARD_REV_NUM >= 3) {
      if (devices[0].controller_data) {
         cs_pin = (unsigned long) devices[0].controller_data;
   
         // All muxData's use same spi_cs_en, so only check 0th device
         if (cs_pin > 256) {
            muxData = (struct AtmelSPIMuxData *)cs_pin;
            muxData->spi_cs_en->enabled = 1;
            muxData = NULL;
	...
	for (i = 0; i < nr_devices; i++) {
		if (devices[i].controller_data) {
			...
			//printk("********  Device %u added with CS pin %lu.\n", i, cs_pin); 
      }
		else {
      ...
      if ( cs_pin > 256 ) {
         muxData = (struct AtmelSPIMuxData *)cs_pin;
         mux = (struct AtmelSPIMux *)(muxData->spi_cs_dev);
         devRdy = (struct AtmelSPIMuxDevReady *)(muxData->spi_dev_ready);
      }
      else {
         mux = NULL;   // Reinitialize mux
         muxData = NULL;
         devRdy = NULL;
      }
      ...
      if (mux) {
         for(; -1 != mux->pin; mux++) {
		      at91_set_gpio_output( mux->pin, 1);
         }
         if (muxData->spi_cs_en->enabled) {
            at91_set_gpio_output(muxData->spi_cs_en->en_pin, 1);
         }
      }
      else
		   at91_set_gpio_output(cs_pin, 1);

      /* enable slave device ready pin */
      if (devRdy)
         at91_set_gpio_input(devRdy->rdy_pin, devRdy->pullup ? 1 : 0);
...
{
	int txpullup = 1, rxpullup = 0;
	if (pins & ATMEL_UART_NO_TX_PULLUP)
		txpullup = 0;
	if (pins & ATMEL_UART_RX_PULLUP)
		rxpullup = 1;

	at91_set_A_periph(AT91_PIN_PB4, txpullup);		/* TXD0 */
	at91_set_A_periph(AT91_PIN_PB5, rxpullup);		/* RXD0 */
...
static inline void configure_usart1_pins(unsigned pins)
{
	...
	at91_set_A_periph(AT91_PIN_PB6, txpullup);		/* TXD1 */
	at91_set_A_periph(AT91_PIN_PB7, rxpullup);		/* RXD1 */
...
static inline void configure_usart2_pins(unsigned pins)
{
	...
	at91_set_A_periph(AT91_PIN_PB8, txpullup);		/* TXD2 */
	at91_set_A_periph(AT91_PIN_PB9, rxpullup);		/* RXD2 */
   printk("UART2 On\n");
...
static inline void configure_usart3_pins(unsigned pins)
{
	...
	at91_set_A_periph(AT91_PIN_PB10, txpullup);		/* TXD3 */
	at91_set_A_periph(AT91_PIN_PB11, rxpullup);		/* RXD3 */
...
static inline void configure_usart4_pins(unsigned pins)
{
	int txpullup = 1, rxpullup = 0;
	if (pins & ATMEL_UART_NO_TX_PULLUP)
		txpullup = 0;
	...
	at91_set_B_periph(AT91_PIN_PA31, txpullup);		/* TXD4 */
	at91_set_B_periph(AT91_PIN_PA30, rxpullup);		/* RXD4 */
}
...
static inline void configure_usart5_pins(unsigned pins)
{
	int txpullup = 1, rxpullup = 0;
	if (pins & ATMEL_UART_NO_TX_PULLUP)
		txpullup = 0;
	...
	at91_set_A_periph(AT91_PIN_PB12, txpullup);		/* TXD5 */
	at91_set_A_periph(AT91_PIN_PB13, rxpullup);		/* RXD5 */
}

static struct platform_device *at91_uarts[ATMEL_MAX_UART];	/* the UARTs to use */
static char at91_uarts_reg[ATMEL_MAX_UART] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
...
void at91_add_device_serial(void)
{
	int i;

	for (i = 0; i < ATMEL_MAX_UART; i++) {
		if (at91_uarts[i] && !at91_uarts_reg[i]) {
...
EXPORT_SYMBOL(at91_add_device_serial);
#else
